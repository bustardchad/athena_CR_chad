...
//  EnrollUserBoundaryFunction(outer_x1, FixMHDRight);
 ...
 //
  ...
  gHeat = pin->GetOrAddReal("problem","gHeat",6.e-25);
  tot_input = pin->GetOrAddReal("problem","dedt",0.0);
...
void Mesh::UserWorkInLoop(void)
{
  ...
  while(pmb != nullptr){
    ...
    for(int k=kl; k<=ku; ++k){
      for(int j=jl; j<=ju; ++j){
        ...
        for(int i=il; i<=iu; ++i){
          tot_cooling += pmb->user_out_var(11,k,j,i) * cell_vol(i);
          tot_heating += pmb->user_out_var(12,k,j,i) * cell_vol(i);
          tot_vol += cell_vol(i);
        }
      }
    }
  ...
  Real diff = (tot_heating + tot_cooling + tot_input*dt)/tot_vol;
  pmb = pblock;
  while(pmb != nullptr){
    int kl = pmb->ks, ku = pmb->ke;
    int jl = pmb->js, ju = pmb->je;
    ...
    for(int k=kl; k<=ku; ++k){
      for(int j=jl; j<=ju; ++j){
        for(int i=il; i<=iu; ++i){
          pmb->user_out_var(13,k,j,i) = diff;
          pmb->user_out_var(14,k,j,i) = diff/dt;
        }
      }
    }
...
void MeshBlock::ProblemGenerator(ParameterInput *pin)
{
 ...
 // std::cout << "sigma in pgen: " << sigmaInput << "\n";
    ...
    for (int j=js; j<=je; ++j) {
      for (int i=is; i<=ie; ++i) {
        ...
        if (NON_BAROTROPIC_EOS){
          phydro->u(IEN,k,j,i) = pthermal/(gamma-1.0);
  ...
  if(CR_ENABLED){
    ...
    Real crBeta = pin->GetOrAddReal("problem","crBeta", 1.e10);  // initial thermal pressure / CR pressure
   // Real ecr0 = 1.e-6;  // initial CR energy density (a small number)
    for(int k=ks; k<=ke; ++k) {
      for (int j=js; j<=je; ++j) {
        for (int i=is; i<=ie; ++i) {
          // pcr->u_cr(CRE,k,j,i) = ecr0;
           pcr->u_cr(CRE,k,j,i) = (pgas/crBeta)/(1./3.);
...
//  Real tot_vol=0.0;
  ...
  Real Tfloor = 1.e4;  
  Real densCeil = 3.066*500.0;  
  Real presFloor = 0.158/1000.0;  
  for(int k=kl; k<=ku; ++k){
    for(int j=jl; j<=ju; ++j){
//      pmb->pcoord->CellVolume(k,j,il,iu,cell_vol);
      for(int i=il; i<=iu; ++i){
       ...
       // if((t_i > cool_t(0)) && (rho < densCeil) && (eint > presFloor/(gamma_idx - 1.0))){  // added density ceiling and pressure floor criteria
        if((t_i > (Tfloor/tunit)) && (rho < densCeil) && (eint > presFloor/(gamma_idx - 1.0))){  // added density ceiling and pressure floor criteria
	 ...
	 */
	 // tnew = std::max(Tfloor/tunit,tnew);  // Took this out August 2022
...
//          tot_cooling += pmb->user_out_var(0,k,j,i) * cell_vol(i);
	 ...
	 // Real gHeat = 6.e-25;   // gives a stable temp (for  n = 1) at T = 8000 K
	 ...
	 // std::cout << "dE/dt_cool: " << (tnew - t_i) * tunit * rhounit * rho/(gamma_idx - 1.0) / (dt * time_unit) << "\n";
          ...
          // cap the internal energy such that temp_cutoff = 3e8 K
   
          pmb->user_out_var(4,k,j,i) = ((tnew - t_i) * rho/(gamma_idx - 1.0))/(dt*heat2code); // + nH*gHeat; // + nH*gHeat; 
          pmb->user_out_var(11,k,j,i) = ((tnew - t_i) * rho/(gamma_idx - 1.0)); // + nH*gHeat; // + nH*gHeat; 
          if (tnew < 3.e8/tunit){ // temperature cap on heating?  Note to self
            cons(IEN,k,j,i) += dt*nH*gHeat*heat2code;
            pmb->user_out_var(4,k,j,i) = ((tnew - t_i) * rho/(gamma_idx - 1.0))/(dt*heat2code) + nH*gHeat; // + nH*gHeat; 
            pmb->user_out_var(12,k,j,i)= dt*nH*gHeat*heat2code;
            //cons(IEN,k,j,i) += dt * rho* heat_coef/(gamma_idx-1.0);
          }else{
            pmb->user_out_var(12,k,j,i) = 0.0;
          } 
...
//    cell_vol.DeleteAthenaArray();
  ...
  for(int k=kl; k<=ku; ++k){
    for(int j=jl; j<=ju; ++j){
      for(int i=il; i<=iu; ++i){
          cons(IEN,k,j,i) += -dt * pmb->user_out_var(14,k,j,i);
      }
    }
