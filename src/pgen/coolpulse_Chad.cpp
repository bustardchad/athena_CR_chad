...
void CRLosses(MeshBlock *pmb, Coordinates *pco, CosmicRay *pcr,
...
void Mesh::InitUserMeshData(ParameterInput *pin)
{
 ...
 //   EnrollUserBoundaryFunction(inner_x1, CRFluxSrcInnerX1);
  ...
  //  EnrollUserCRSource(CRLosses); //? will this work?
 // }
  ...
  for (int k=pmb->ks; k<=pmb->ke; ++k) {
    for (int j=pmb->js; j<=pmb->je; ++j) {
      for (int i=pmb->is; i<=pmb->ie; ++i) {
        ...
        Real dedt_coul = 2.78e-16 * ne * u_cr(CRE,k,j,i) * dcgs
        Real dedt_pion = 7.44e-16 * ne * u_cr(CRE,k,j,i) * dcgs
        u_cr(CRE,k,j,i) -= dt*(dedt_pion + dedt+coul)
