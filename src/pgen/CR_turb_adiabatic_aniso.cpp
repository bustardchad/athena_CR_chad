...
void MeshBlock::ProblemGenerator(ParameterInput *pin)
{
 ...
 // Real iso_cs = pin->GetOrAddReal("problem","iso_sound_speed", .011);      // gas density outside cloud
  Real iso_cs = pin->GetOrAddReal("problem","iso_sound_speed", .11);      // sound speed initially
  Real pthermal = pgas;
    ...
    for (int j=js; j<=je; ++j) {
      for (int i=is; i<=ie; ++i) {
        ...
        if (NON_BAROTROPIC_EOS){
          phydro->u(IEN,k,j,i) = density*iso_cs*iso_cs/(gamma-1.0); // pressure is same as isothermal case
      ...
      for (int j=js; j<=je; ++j) {
        for (int i=is; i<=ie; ++i) {
          ...
          // pcr->u_cr(CRE,k,j,i) = (pgas/crBeta)/(1./3.);
           pcr->u_cr(CRE,k,j,i) = (rho_h*iso_cs*iso_cs/crBeta)/(1./3.);
