...
void MeshBlock::ProblemGenerator(ParameterInput *pin)
{
 ...
 // std::cout << "sigma in pgen: " << sigmaInput << "\n";
  ...
  Real z_cloud = pin->GetOrAddReal("problem","zcloud", 0.);     // y-coordinate of cloud center
  ...
  Real z_cloud2 = pin->GetOrAddReal("problem","zcloud2", 0.);     // z-coordinate of cloud center
  ...
  for(int k=ks; k<=ke; ++k) {
    for (int j=js; j<=je; ++j) {
      for (int i=is; i<=ie; ++i) {
        ...
        Real x1_b = pcoord->x1v(i) - x_cloud2;
        Real x2_b = pcoord->x2v(j) - y_cloud2;
        Real x3_b = pcoord->x3v(k) - z_cloud2;
        Real r2_b = x1_b*x1_b + x2_b*x2_b + x3_b*x3_b;
        Real r_b  = sqrt(r2_b);

        Real density = rho_h + (rho_c - rho_h) * 0.5 *
                               (1.0 - 1.0*tanh((r-r_cloud)/delta_r));
        // add in 2nd cloud
        density = density + (rho_c - rho_h) * 0.5 *
