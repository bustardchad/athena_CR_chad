            ...
            if (pcr->ion_neutral_friction > 0.) {
              if (fneutral > TINY_NUMBER) {
               // std::cout << "sigma_diff: " << 1./damping_diff_term << " k parallel: " << damping_diff_term << "\n";
                pcr->sigma_diff(0,k,j,i) = 1./(std::min(damping_diff_term, 1.E0));  // Chad, March 28, 2020
  ...
  for (int k=pmb->ks; k<=pmb->ke; ++k) {
    for (int j=pmb->js; j<=pmb->je; ++j) {
      for (int i=pmb->is; i<=pmb->ie; ++i) {
        ...
        Real temp = miu*mp*pcgs/(dcgs*kb);      // temperature in Kelvin
        Real nH = 0.7*dcgs/mp;                 // proton density for fully ionized medium with ~25% Helium by mass

        Real lambda = 0.0;
        if (temp < Tfloor) { lambda = 0.; }
	else if (temp < 1.e5) {lambda = 4.6240e-36 * pow(temp,2.867);}
	else if (temp < 4.e7) {lambda = 1.780e-18 * pow(temp,-0.65);}
	else {lambda = 0.0;}
