...
void Mesh::InitUserMeshData(ParameterInput *pin)
{
 ...
 //   EnrollUserBoundaryFunction(inner_x1, CRFluxSrcInnerX1);
  ...
  EnrollFieldDiffusivity(SpatialDiffusivity);
  ...
  for(int k=ks; k<=ke; ++k) {
    for (int j=js; j<=je; ++j) {
      for (int i=is; i<=ie; ++i) {
        ...
        phydro->u(IM2,k,j,i) = density*0.01; // velocity is 1 kpc in 100 Myrs
...
#pragma omp simd
      ...
      // std::cout << "fneutral: " << fneutral << " maxNeut: " << pcr->maxNeut << " tanh term: " << tanh((-temp + tempCutoff)/(coeff)) << "\n";
          ...
          Real dampRate = 1.e-9 * fneutral * pow((temp / 1000.0),0.5) * dcgs/1.e-24;
        //  Real collFreq = max(1.E-20,2.*dampRate); //2 x damping rate and put into code units
          Real collFreq = max(1.E-20,2.*dampRate*3.155e13); //2 x damping rate and put into code units
	  Real kludge = 1.0;
	 // Real etaad = kludge*(SQR(bmag(k,j,i)*pres2cgs/2.)*(1.0 + tanh(1.5*(-temp + tempCutoff)/coeff))/(3.e29*fion*collFreq*dcgs));
	  Real etaad = kludge*(SQR(bmag(k,j,i))*(1.0 + tanh(1.5*(-temp + tempCutoff)/coeff))/(fion*collFreq*w(IDN,k,j,i)));
	 // if (fneutral > 0) {
	 //   std::cout << "B^2: " << SQR(bmag(k,j,i)) << " collFreq: " << collFreq << " fneutral: " << fneutral << " temp: " << temp << " density: " << dcgs << "\n";
	 // }
	 // std::cout << "eta ad: " << etaad << "\n";
	  pmb->user_out_var(8,k,j,i) = etaad; // ambipolar diffusivity in units of 3e29 cm^2/s
          pfdif->etaB(FieldDiffusion::DiffProcess::ambipolar, k,j,i) = etaad*SQR(bmag(k,j,i));
