...
Real etaInject, epsShock, beta, delta;
bool turbClumps;
  ...
  for(int k=ks; k<=ke; ++k) {
    for (int j=js; j<=je; ++j) {
      for (int i=is; i<=ie; ++i) {
        ...
        Real xcoordL = pcoord->x1f(i);
        Real xcoordR = pcoord->x1f(i+1);
...
// !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  //         ! put in a shock 
  	if (xcoordR < shockPos) { // downstream
  		rhoZone = rhoDown;
        	presZone = presDown;
        	velxZone = -velDown;
        	BZone = BDown;
 	}
	else if ((xcoordL < shockPos) && (xcoordR > shockPos)) { //in shock
  	  presZone = (presUp + presDown)/2.;
  	  velxZone = -(velUp+velDown)/2.;
          BZone = (B0+BDown)/2.;
  	  rhoZone = (density+rhoDown)/2.; // changed to density, already defined above
	}
  	else { //upstream
  	  presZone = presUp;
  	  velxZone = -velUp;
  ...
  for (int j=pmb->js; j<=pmb->je; j++) {
    for (int i=pmb->is; i<=pmb->ie; i++) {
     ...
     // Real minP = std::min(prim(IPR,k,j-1:j+1,i-1:i+1));
      ...
      if ((std::abs(gradPx) + std::abs(gradPy)) > beta*minP) {SW1 = true;} 
