...
void Mesh::InitUserMeshData(ParameterInput *pin)
{
 ...
 if(CR_ENABLED){
    ...
    */
    ...
    time_unit = 3.155e14; // 1 Myr is how Josh set it up?? Note to self
...
//      pmb->pcoord->CellVolume(k,j,il,iu,cell_vol);
	...
	//  std::cout << "What it wants tnew to be: " << tnew*tunit << "\n";
	  tnew = std::max(Tfloor/tunit,tnew);
          pmb->user_out_var(4,k,j,i) = (tnew - t_i) * tunit * rhounit * rho/(gamma_idx - 1.0);
    ...
    for(int k=kl; k<=ku; ++k){
	  ...
	  fion = std::max(1.e-4, fion);
          ...
          Real va = sqrt(pb/prim(IDN,k,j,i));
            ...
            Real lcr = 1./3.*prim(IDN,k,j,i)*u_cr(CRE,k,j,i) / sqrt(pow(pcr->b_grad_pc(k,j,i),2.)); // + pow(pcr->b_grad_pc(1,k,j,i),2.) + pow(pcr->b_grad_pc(2,k,j,i),2.));
            lcr *= sqrt(pb); 
           // lcr *= lcgs;
	    ...
	    damping_diff_term *= va/sqrt(fion);
                        //      /(4.803e-10*sqrt(4*3.1415*pb*pres2cgs)*3.1415*(pow(va,2.0)*pres2cgs/dens2cgs)*prim(IDN,k,j,i)*cr(CRE,k,j,i)*pres2cgs));

           /*
           if (stream_boost_factor > 10000.0) {
                std::cout << "||  density: " << dcgs << "  temperature: " << temp << "  eb: " << 0.5*pb << "  ecr: " << w(IDN,k,j,i)*cr(CRE,k,j,i);
                std::cout << "fneutral: " << fneutral << "  va: " << va << "  damp rate: " << dampRate << "  boost factor: " << stream_boost_factor;
           }
           */

	  pcr->sigma_diff(0,k,j,i) = pcr->max_opacity;
	  if (pcr->ion_neutral_friction > 0) {
            pcr->sigma_diff(0,k,j,i) += std::max(1./damping_diff_term, 1.E0);  // Chad, March 28, 2020
	  }
          pcr->sigma_diff(1,k,j,i) = pcr->max_opacity;
          pcr->sigma_diff(2,k,j,i) = pcr->max_opacity;  

          if (pcr->ion_neutral_friction > 0) { 
	    va *= std::min(1./sqrt(fion),1.e2);  // Chad, March 28, 2020
	    va1 *= std::min(1./sqrt(fion),1.e2);  // Chad, March 28, 2020
	    va2 *= std::min(1./sqrt(fion),1.e2);  // Chad, March 28, 2020
	    va3 *= std::min(1./sqrt(fion),1.e2);  // Chad, March 28, 2020

            va1 = std::min(va1,1.e0);  // so now the ion alfven velocity is capped at ~ 1.0 code units
            va2 = std::min(va2,1.e0);
            va3 = std::min(va3,1.e0);
