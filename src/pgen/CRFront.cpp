...
// use X=0.7, Z=0.02
static  Real miu =0.617284;
static Real miue=1.17647;
static Real miuh=1.42857;
static Real kb=1.3807e-16;
static Real mp=1.6605e-24;
    
static Real dens2cgs = 6.85e-27;
static Real pres2cgs = 6.54e-11;
static Real heat2code = 4.81e23;
static Real narr_len;
...
void Mesh::InitUserMeshData(ParameterInput *pin)
{
 ...
 heat_coef = pin->GetOrAddReal("problem","hgamma",1.e-26);
// heat_coef = heat_coef*4.81e23;
 narr_len = pin->GetOrAddReal("problem","narr_len",1.e4);
 pgascold  = pin->GetOrAddReal("problem","pgascold", 2.11E-3);         // gas pressure in cold cloud
 xcrcold = pin->GetOrAddReal("problem","xcrcold",  10.);     // Pcrcold/Pgcold
 
    cool_t.NewAthenaArray(nline);
    cool_coef.NewAthenaArray(nline);
    lam_coef.NewAthenaArray(nline);
   ...
   // tunit = mp*pres2cgs/(rhounit*kb); // = 1.15E8 K
    ...
    lam_coef(0) = 1.0012e-30; //* pow(2000.0,1.5);
    lam_coef(1) = 4.6240e-36; //* pow(8000.0,2.867);
    lam_coef(2) = 1.780e-18; //* pow(1.e5,-0.65);
 ...
 // for the steady-state front solution
 //
 
 
  // get parameters from input file
  Real T0  = pin->GetOrAddReal("problem","T0", 1.41E-4);         // reference temp = 1e4 K in code units
  Real bx = pin->GetOrAddReal("problem","bx", .0349);
  Real by = pin->GetOrAddReal("problem","by", 0.);
  Real bz = pin->GetOrAddReal("problem","bz", 0.); 
  Real xmax = pin->GetOrAddReal("problem","xmax", 1.);  // in kpc
  Real ptot = pgascold*(1.0+xcrcold); // in code units
 // Real n0 = pgascold*pres2cgs/(kb*T0*tunit) / (dens2cgs/(miu*mp)); // in code units
  n0 = pgascold*pres2cgs/(kb*T0*tunit)/dens2cgs; // in code units
  Real pc0 = xcrcold*pgascold; // in code units
  narr.NewAthenaArray(int(narr_len));
  v.NewAthenaArray(int(narr_len));
  narr(0) = n0;
  Real deltax = xmax/narr_len;
  
  for(int i = 0; i<int(narr_len); i++){
        v(i) = deltax*i;
        Real ni = narr(i);
        Real ni_cgs = ni*dens2cgs;
        Real t_i = (ptot - pc0*pow(ni/n0,2./3.))*pres2cgs/(kb*ni*tunit*dens2cgs); // in code units
        Real slope = cool_index(0);
        Real coef = lam_coef(0);
        int t_loc = 0;
        if(t_i > cool_t(0)){
          ...
          slope = cool_index(t_loc);
          coef = lam_coef(t_loc);
        }
        Real Lambda_i = coef * pow(t_i*tunit,slope); // Lambda(T)
       // cout << "ti: " << t_i << "Lambda_i: " << Lambda_i << "\n";
        Real density = ni*miu*mp;
        Real Pc = pc0*pow(ni/n0,2./3.);
        Real CRHeat = (bx/sqrt(density))*Pc;
...
void MeshBlock::ProblemGenerator(ParameterInput *pin)
{
 ...
 // Real pgascold  = pin->GetOrAddReal("problem","pgascold", 2.11E-3);         // gas pressure in cold cloud
  Real T0  = pin->GetOrAddReal("problem","T0", 1.41E-4);         // reference temp = 1e4 K in code units
 // Real xcrcold = pin->GetOrAddReal("problem","xcrcold",  10.);     // Pcrcold/Pgcold
  Real bx = pin->GetOrAddReal("problem","bx", 1.);
  Real by = pin->GetOrAddReal("problem","by", 0.);
  Real bz = pin->GetOrAddReal("problem","bz", 0.); 
  Real xmax = pin->GetOrAddReal("problem","xmax", 1.);  // in kpc
  Real ptot = pgascold*(1.0+xcrcold);
  n0 = pgascold*pres2cgs/(kb*T0*tunit) / dens2cgs;
  Real pc0 = xcrcold*pgascold;
  
 // int vlen = sizeof(v) / sizeof(v[0]);
 // std::vector<double> vect(v,v+vlen);
  ...
  for(int k=ks; k<=ke; ++k) {
    for (int j=js; j<=je; ++j) {
      for (int i=is; i<=ie; ++i) {
        ...
        while((xit < int(narr_len)) && (v(xit) < x1) ){
            ++xit;
          }
        //int xit = std::lower_bound(vect.begin(),vect.end(),x1);
        Real ndens = narr(xit);        
        Real Pcr = pc0*pow(ndens/n0,2./3.);
        Real Pg = ptot - Pcr;
        phydro->u(IDN,k,j,i) = ndens*miu*mp;
        ...
        if(CR_ENABLED){
           pcr->u_cr(CRE,k,j,i) = Pcr/(4./3. - 1.0);
      ...
      for(int j=0; j<nz2; ++j){
        for(int i=0; i<nz1; ++i){
         // pcr->sigma_diff(0,k,j,i) = sigmaInput;
          pcr->sigma_diff(0,k,j,i) = (3.0/pcr->vmax)*1./(pcr->diffInput);
...
//      pmb->pcoord->CellVolume(k,j,il,iu,cell_vol);
      for(int i=il; i<=iu; ++i){
        ...
        Real eintFloor = (Tfloor/tunit) * rho / (gamma_idx - 1.0);
	 ...
	 */
	 // tnew = std::max(Tfloor/tunit,tnew);
         ...
         // cons(IEN,k,j,i) += (tnew - t_i) * rho/(gamma_idx - 1.0);
          eint += (tnew - t_i) * rho/(gamma_idx - 1.0);
...
//          tot_cooling += pmb->user_out_var(0,k,j,i) * cell_vol(i);
	 ...
	 // std::cout << "dE/dt_cool: " << (tnew - t_i) * tunit * rhounit * rho/(gamma_idx - 1.0) / (dt * time_unit) << "\n";
         ...
         // cons(IEN,k,j,i) += dt*nH*heat_coef;
          //cout << "Temp: " << tnew*tunit << " Cooling: " << (tnew - t_i) * rho/(gamma_idx - 1.0) << " Heating: " << dt*nH*heat_coef*heat2code << "\n";

	  cons(IEN,k,j,i) = std::max(eintFloor,eint) 
                      + 0.5 *(cons(IM1,k,j,i)*cons(IM1,k,j,i)
                      + cons(IM2,k,j,i)*cons(IM2,k,j,i)
                      + cons(IM3,k,j,i)*cons(IM3,k,j,i))/rho;
          if(MAGNETIC_FIELDS_ENABLED){
             cons(IEN,k,j,i) += 0.5 * (bcc(IB1,k,j,i) * bcc(IB1,k,j,i)
                      + bcc(IB2,k,j,i) * bcc(IB2,k,j,i)
                      + bcc(IB3,k,j,i) * bcc(IB3,k,j,i));
          }

         // pmb->user_out_var(4,k,j,i) = - (tnew - t_i) * tunit * rhounit * rho/(gamma_idx - 1.0) / (dt * time_unit); // + nH*gHeat; 
          pmb->user_out_var(4,k,j,i) = ((tnew - t_i) * rho/(gamma_idx - 1.0))/(dt*heat2code) + (nH*heat_coef); // + nH*gHeat; 
...
#pragma simd
          for (int i=1; i<=(NGHOST); ++i) {
            u_cr(n,k,j,is-i) = xcrcold*pgascold/(4./3. - 1.0);
           // u_cr(CRF1,k,j,is-i) = u_cr(CRF1,k,j,is);  // set x-crflux to set value
          }
        }}
      }
      else {
...
#pragma simd
          for (int i=1; i<=(NGHOST); ++i) {
            ...
            //u_cr(n,k,j,is-i) = xcrcold*pgascold/(4./3. - 1.0);  // copy everything else as for hydro variables
          }
        }}
      }
    }
    ...
    for (int j=js; j<=je; ++j) {
      for (int i=1; i<=ngh; ++i) {
       ...
       // prim(IDN,k,j,is-i) = prim(IDN,k,j,is);
        prim(IDN,k,j,is-i) = n0*miu*mp;
       // cout << "boundary density: " << prim(IDN,k,j,is-i) << "\n";
       // prim(IVX,k,j,is-i) = -prim(IVX,k,j,is); // reflect 1-velocity
        prim(IVX,k,j,is-i) = prim(IVX,k,j,is); // reflect 1-velocity
        prim(IVY,k,j,is-i) = prim(IVY,k,j,is);
        prim(IVZ,k,j,is-i) = prim(IVZ,k,j,is);
        if(NON_BAROTROPIC_EOS)
          prim(IEN,k,j,is-i) = pgascold; // Note that primitive variable holds pressure, not energy, so no gamma - 1 factor here
//          prim(IEN,k,j,is-i) = 10.0;
//         prim(IEN,k,j,is-i) = prim(IEN,k,j,is+i-1);
       // cout << "boundary gas pressure: " << prim(IEN,k,j,is-i) << "\n";
      }
    }
  }
