...
void Mesh::InitUserMeshData(ParameterInput *pin)
{
 ...
 int sim_kx1 = pin->GetOrAddReal("problem","sim_kx1", 10);
 int sim_ky1 = pin->GetOrAddReal("problem","sim_ky1", 10);
 int sim_kx2 = pin->GetOrAddReal("problem","sim_kx2", 50);
 int sim_ky2 = pin->GetOrAddReal("problem","sim_ky2", 50);
 Real alpha_exp = pin->GetOrAddReal("problem","alpha_exp", 0.01);
 Real L = pin->GetOrAddReal("problem","L", 8.E23);

// AllocateRealUserMeshDataField(4); 
// AthenaArray<Real> Phii1Vec;
// AthenaArray<Real> Phii2Vec;
// AthenaArray<Real> Phii3Vec;
// AthenaArray<Real> AiiVec;
...
// ruser_mesh_data[3].NewAthenaArray(sim_kx2,sim_ky2,sim_ky2);
 ...
 for (int kx = 0; kx<sim_kx2; ++kx) {
   for (int ky = 0; ky<sim_ky2; ++ky) {
      for (int kz = 0; kz<sim_ky2; ++kz) { // use the same for y and z ! Chad -- 2/11/18
...
/* AllocateRealUserMeshBlockDataField(4);
 Phii1MeshVec.NewAthenaArray(sim_kx2,sim_ky2,sim_ky2);
 Phii2MeshVec.NewAthenaArray(sim_kx2,sim_ky2,sim_ky2);
 Phii3MeshVec.NewAthenaArray(sim_kx2,sim_ky2,sim_ky2);
 AiiMeshVec.NewAthenaArray(sim_kx2,sim_ky2,sim_ky2);
 ...
 for (int kx = sim_kx1; kx<=sim_kx2; ++kx) {
          for (int ky = sim_ky1; ky<=sim_ky2; ++ky) {
            for (int kz = sim_ky1; kz<=sim_ky1; ++kz) { // use the same for y and z
                AiiMeshVec(kx,ky,kz) = AiiVec(kx,ky,kz);
                Phii1MeshVec(kx,ky,kz) = Phii1Vec(kx,ky,kz);
                Phii2MeshVec(kx,ky,kz) = Phii2Vec(kx,ky,kz);
                Phii3MeshVec(kx,ky,kz) = Phii3Vec(kx,ky,kz);
            }
          }
        }
 ...
 // std::cout << "sigma in pgen: " << sigmaInput << "\n";
  ...
  for(int k=ks; k<=ke; ++k) {
    ...
    //    int kz = sim_ky1;
        for (int kx = sim_kx1; kx<sim_kx2; ++kx) {
          for (int ky = sim_ky1; ky<sim_ky2; ++ky) {
            for (int kz = sim_ky1; kz<sim_ky2; ++kz) {
                fnew = fnew + AiiVec(kx,ky,kz)*sin((kx*xcoord*2.0*pi/L+Phii1Vec(kx,ky,kz)))*sin(ky*ycoord*2.0*pi/L + Phii2Vec(kx,ky,kz))*sin(kz*zcoord*2.0*pi/L+Phii3Vec(kx,ky,kz));
  //              fnew = fnew + AiiMeshVec(kx,ky,kz)*sin((kx*xcoord*2.0*pi/L+Phii1MeshVec(kx,ky,kz)))*sin(ky*ycoord*2.0*pi/L + Phii2MeshVec(kx,ky,kz)); //*sin(kz*zcoord*2.0*pi/L+Phii3Vec(kx,ky,kz));
               // fnew = fnew + ruser_mesh_data[3](kx,ky,kz)*sin((kx*xcoord*2.0*pi/L+ruser_mesh_data[0](kx,ky,kz)))*sin(ky*ycoord*2.0*pi/L + ruser_mesh_data[1](kx,ky,kz)); //*sin(kz*zcoord*2.0*pi/L+Phii3Vec(kx,ky,kz));
	//        std::cout << "kx: " << kx << " ky: " << ky << " kz: " << kz << " fnew: " << fnew << "\n";
            }
          }
        }
        
	
       // Real density = rho_h*exp(alpha_exp*fnew);
       // Real density = rho_h*exp(-xcoord/scaleheight)*exp(alpha_exp*fnew*exp(-xcoord/scaleheight));
        Real density = rho_h*exp(-xcoord/scaleheight)*exp(alpha_exp*fnew*(1.0-exp(-xcoord))*exp(-xcoord/scaleheight));
       // Real density = rho_h; 
	// std::cout << density << "\n";
        ...
        for (int i=1; i<=(NGHOST); ++i) {
          ...
          if(n==CRE) {
           // u_cr(n,k,j,ie+i) = 0.99999999*u_cr(n,k,j,ie);
            u_cr(n,k,j,ie+i) = 0.9*u_cr(n,k,j,ie);
          } else {
