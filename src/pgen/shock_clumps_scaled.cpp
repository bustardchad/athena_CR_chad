...
namespace {
Real presUp, rhoUp, velUp, B0, BAngle, shockPos, Mach;
Real L, alpha_exp;
int sim_kx1, sim_ky1, sim_kx2, sim_ky2;
Real etaInject, epsShock, beta, betaCR, delta;
bool turbClumps;
...
void CRInject(MeshBlock *pmb, const Real time, const Real dt, const AthenaArray<Real> &prim, 
     const AthenaArray<Real> &bcc, AthenaArray<Real> &u_cr, AthenaArray<Real> &cons);

void CRInjectFlash(MeshBlock *pmb, const Real time, const Real dt, const AthenaArray<Real> &prim, 
     const AthenaArray<Real> &bcc, AthenaArray<Real> &u_cr, AthenaArray<Real> &cons);
...
void Mesh::InitUserMeshData(ParameterInput *pin)
{
 ...
 // AthenaArray<Real> fboundary, Phii1Vec, Phii2Vec, Phii3Vec, AiiVec; // here too, so they
  ...
  etaInject = pin->GetOrAddReal("problem","etaInject", 5.E-4);
  epsShock = pin->GetOrAddReal("problem","epsShock", 0.1);
  beta = pin->GetOrAddReal("problem","beta", 1.0);
  betaCR = pin->GetOrAddReal("problem","betaCR", 1.0);
  delta = pin->GetOrAddReal("problem","delta", 0.1); 
  shockPos = pin->GetOrAddReal("problem","shockPos", 0.0);
  Mach = pin->GetOrAddReal("problem","Mach", 4.5);
//  Real sigma = pin->GetOrAddReal("problem","sigma", 1.e30);
...
void MeshBlock::InitUserMeshBlockData(ParameterInput *pin)
{
   ...
   // AllocateUserOutputVariables(2);
    if(CR_ENABLED){
     ...
     // pcr->EnrollUserCRGasSource(CRInject);
      pcr->EnrollUserCRGasSource(CRInjectFlash);
    }
 // if(CR_ENABLED){
...
void MeshBlock::ProblemGenerator(ParameterInput *pin)
{
 ...
 // Real presUp = pin->GetOrAddReal("problem","presUp", 1.);
  Real presUp = 1.0/gamma;
  Real Mach = pin->GetOrAddReal("problem","Mach", 4.5);
 ...
 // std::cout << "Mach: " << Mach << "factor1: " << factor1;
  // !!!!!!!!!!!!!!!!!
  if (magJump == true) {
  /*
...
*/
	...
	Real root = 1.;
       // !instead, use quadratic formula from UT farside website
        if (BAngle == 90.0) {
             // cout << "BAngle = 90";
               ...
               // Real beta = 8.0*pi*presUp/pow((B0*cos(BAngle*pi/180.)),2.0);
                Real beta = 2.0*presUp/pow((B0*cos(BAngle*pi/180.)),2.0);
                Real a = 2.0*(2. - gamma);
        ...
        rhoDown = factor1 * rhoUp;
        velDown = velUp/factor1;
       // velDown = velUp + jump3*Mach*std::sqrt(gamma*presUp/rhoUp);
        presDown = factor2 * presUp;
        BDown = B0;
  }
  else  {
...
// !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
 ...
 //          pcr->q_cr(k,j,i) = 0.0;                            // no explicit CR source
  ...
  if(MAGNETIC_FIELDS_ENABLED){
   ...
   // Real bz = 0.;
   //
   Real bxl = B0*sin(BAngle*pi/180.);
   Real bxr = B0*sin(BAngle*pi/180.);

   Real byl = BDown*cos(BAngle*pi/180.);
   Real byr = B0*cos(BAngle*pi/180.);
 ...
 //   pfield->CalculateCellCenteredField(pfield->b,pfield->bcc,pcoord,is,ie,js,je,ks,ke);
    ...
    for(int k=ks; k<=ke; ++k){
      for(int j=js; j<=je; ++j){
        for(int i=is; i<=ie; ++i){
	  if (pcoord->x1v(i) < shockPos) {
            phydro->u(IEN,k,j,i) += 0.5*(bxl*bxl + byl*byl);
          } else {
            phydro->u(IEN,k,j,i) += 0.5*(bxr*bxr + byr*byr);
          }
         // phydro->u(IEN,k,j,i) +=
...
void CRInject(MeshBlock *pmb, const Real time, const Real dt, const AthenaArray<Real> &prim, 
     const AthenaArray<Real> &bcc, AthenaArray<Real> &u_cr, AthenaArray<Real> &cons) {
  Real rampup = 1. - exp(-time/1.E12);
 // std::cout << "CRInject function ";
 // CosmicRay *pcr=pmb->pcr;
 // AthenaArray<Real> &w = pmb->phydro->w;
  int k = pmb->ks;
  bool okShock = false;
    ...
    for (int i=pmb->is; i<=pmb->ie; i++) {
     ...
     /* Real epsr = (std::abs(w(IDN,k,j,i+1) - 2.0*w(IDN,k,j,i) + w(IDN,k,j,i-1))
      ...
      Real epsp = (std::abs(w(IPR,k,j,i+1) - 2.0*w(IPR,k,j,i) + w(IPR,k,j,i-1))
                  + std::abs(w(IPR,k,j+1,i) - 2.0*w(IPR,k,j,i) + w(IPR,k,j-1,i)))
                  /w(IPR,k,j,i); */
      Real epsr = (std::abs(prim(IDN,k,j,i+1) - 2.0*prim(IDN,k,j,i) + prim(IDN,k,j,i-1))
                  + std::abs(prim(IDN,k,j+1,i) - 2.0*prim(IDN,k,j,i) + prim(IDN,k,j-1,i)))
                  /prim(IDN,k,j,i);
      Real epsp = (std::abs(prim(IPR,k,j,i+1) - 2.0*prim(IPR,k,j,i) + prim(IPR,k,j,i-1))
                  + std::abs(prim(IPR,k,j+1,i) - 2.0*prim(IPR,k,j,i) + prim(IPR,k,j-1,i)))
                  /prim(IPR,k,j,i);
      Real eps = std::max(epsr, epsp);
     // maxeps = std::max(maxeps, eps);
 ...
 // AthenaArray<Real> &w = pmb->phydro->w;
  ...
  for (int j=pmb->js; j<=pmb->je; j++) {
    for (int i=pmb->is; i<=pmb->ie; i++) {
      ...
      bool SW2 = false;
      bool okShock = false;
      Real minC = std::min({sqrt(5./3.*prim(IPR,k,j-1,i-1)/prim(IDN,k,j-1,i-1)),sqrt(5./3.*prim(IPR,k,j-1,i)/prim(IDN,k,j-1,i)),
             sqrt(5./3.*prim(IPR,k,j-1,i+1)/prim(IDN,k,j-1,i+1)), sqrt(5./3.*prim(IPR,k,j,i-1)/prim(IDN,k,j,i-1)), 
             sqrt(5./3.*prim(IPR,k,j+1,i-1)/prim(IDN,k,j+1,i-1)), sqrt(5./3.*prim(IPR,k,j,i)/prim(IDN,k,j,i)), 
             sqrt(5./3.*prim(IPR,k,j+1,i)/prim(IDN,k,j+1,i)), sqrt(5./3.*prim(IPR,k,j+1,i+1)/prim(IDN,k,j+1,i+1))});
      Real dx = (pmb->pcoord->x1v(i)) - (pmb->pcoord->x1v(i-1));
      Real divv = (prim(IVX,k,j,i+1) - prim(IVX,k,j,i-1))
                  + (prim(IVY,k,j+1,i) - prim(IVY,k,j-1,i));
                 // + (prim(vz,k+1,j,i) - prim(vz,k-1,j,i));
      divv = 0.5*divv;
      
      Real gradPx = 0.5*(prim(IPR,k,j,i+1) - prim(IPR,k,j,i-1));
      Real gradPy = 0.5*(prim(IPR,k,j+1,i) - prim(IPR,k,j-1,i));
     // Real gradPz = 0.5*(prim(IPR,k+1,j,i) - prim(IPR,k-1,j,i));
       
     // Real minP = std::min(prim(IPR,k,j-1:j+1,i-1:i+1));
      Real minP = std::min({prim(IPR,k,j-1,i-1), prim(IPR,k,j-1,i), prim(IPR,k,j-1,i+1), prim(IPR,k,j,i-1),
		  prim(IPR,k,j+1,i-1), prim(IPR,k,j,i), prim(IPR,k,j+1,i), prim(IPR,k,j+1,i+1)});

      if ((std::abs(gradPx) + std::abs(gradPy)) > betaCR*minP) {SW1 = true;} 
      if (-delta*minC > divv) {SW2 = true;} 	          
     // maxeps = std::max(maxeps, eps);
    
      if (SW1 == true && SW2 == true) {okShock = true;}	
