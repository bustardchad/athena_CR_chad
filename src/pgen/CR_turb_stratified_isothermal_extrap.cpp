...
// New parameter inputs for the stratified case:
// window1 and window2 for where the turbulent stirring should occur within the boundaries
// grav_acc -- in code units
// rhohot --> rho_h is the density in the midplane
// iso_cs is the isothermal sound speed
// crBeta = ratio of thermal pressure to cosmic ray pressure at the midplane
	// All other parameters are determined from these

// Other additions:
//      BC: outer (z direction) pressure boundary condition helps ensure HSE by reflecting 
//      z velocity and integrating the thermal pressure into the ghost cells.
//      Cosmic rays are handled in a different BC and make the CR pressure in the ghost cells
//      a little lower than in the last cell before the boundary
//
// Inner boundaries are fixed CR pressure, gas pressure, density     
...
static AthenaArray<Real> Phii1Vec, Phii2Vec, Phii3Vec, AiiVec; // here too, so they
static Real L, alpha_exp;
static int sim_kx1, sim_ky1, sim_kx2, sim_ky2;

static Real pi = 3.14159265358979323846;
static Real sigma=1.e30; // some large number to prevent diffusion (large enough?)

static int n_user_var=11;
...
static AthenaArray<Real> v, rhoarr;
static int nline=4;
...
static Real rhoarr_len;
static Real pc0, rho_h;
static Real crBeta, zmax;
...
void CROutflowOuterX3(MeshBlock *pmb, Coordinates *pco, CosmicRay *pcr,
    const AthenaArray<Real> &w, const AthenaArray<Real> &bcc,
    AthenaArray<Real> &u_cr, Real time, Real dt, int is, int ie,
    int js, int je, int ks, int ke, int ngh);

void CRGradientOuterX3(MeshBlock *pmb, Coordinates *pco, CosmicRay *pcr,
    const AthenaArray<Real> &w, const AthenaArray<Real> &bcc,
    AthenaArray<Real> &u_cr, Real time, Real dt, int is, int ie,
    int js, int je, int ks, int ke, int ngh);
...
void Mesh::InitUserMeshData(ParameterInput *pin)
{
 // EnrollUserBoundaryFunction(outer_x3, ProjectPressureOuterX3);
  EnrollUserBoundaryFunction(outer_x3, FixMHDRight);
  EnrollUserBoundaryFunction(inner_x3, FixMHDLeft);
  if(CR_ENABLED) {
    EnrollUserCRBoundaryFunction(inner_x3, FixCRsourceLeft);
   // EnrollUserCRBoundaryFunction(outer_x3, CROutflowOuterX3);
    EnrollUserCRBoundaryFunction(outer_x3, CRGradientOuterX3);
  }
 //
 //
  gamma_idx = pin->GetOrAddReal("hydro", "gamma", 5.0/3.0);
  iso_cs = pin->GetOrAddReal("hydro", "iso_sound_speed", 0.011);
  sigmaInput = pin->GetOrAddReal("problem","sigmaInput",1.e30);
 // grav_acc = pin->GetOrAddReal("problem","grav_acc",1.e-30);
  grav_acc = pin->GetOrAddReal("hydro","grav_acc3",-1.e-30);
  rhoarr_len = pin->GetOrAddReal("problem","rhoarr_len",1.e3);
...
// if(CR_ENABLED){
  ...
  Real bx = pin->GetOrAddReal("problem","bx", .0349);
  Real by = pin->GetOrAddReal("problem","by", 0.);
  Real bz = pin->GetOrAddReal("problem","bz", 0.);
  zmax = pin->GetOrAddReal("problem","zmax", 2.);  // in kpc
  rho_h = pin->GetOrAddReal("problem","rhohot", .01);      // gas density at midplane
  crBeta = pin->GetOrAddReal("problem","crBeta", 1.E10);      //  initial thermal pressure / CR pressure
  

  rhoarr.NewAthenaArray(int(rhoarr_len));
  v.NewAthenaArray(int(rhoarr_len));
  rhoarr(0) = rho_h;
  Real deltaz = (1.2*zmax)/rhoarr_len;
  pc0 = rho_h*pow(iso_cs,2.)/crBeta;
  for(int i = 0; i<int(rhoarr_len); i++){
        v(i) = deltaz*i;
        Real rhoi = rhoarr(i);
        Real A_const = (2./3.)*pc0/(pow(rho_h,2./3.));
        Real Pc = pc0*pow(rhoi/rho_h,2./3.);
        Real CRHeat = (bx/sqrt(rhoi))*Pc;
        Real drhodzi = (grav_acc*rhoi)/(A_const*pow(rhoi,-1./3.) + pow(iso_cs,2.));
       // Real dndxi = ni*(2.0/(4./3.))*(-pow(ni_cgs*miu,2.0)*Lambda_i/(miuh*miue) + (ni_cgs*heat_coef))*heat2code/CRHeat;
       // cout << "n^2Lambida_i: " << pow(ni_cgs,2.0)*Lambda_i << "\n";
       // cout << "ni*heat_coef" << ni_cgs*heat_coef << "CRHeat: " << CRHeat/heat2code << "\n"; 
        Real rho_i1 = rhoi + drhodzi*deltaz;
        rhoarr(i+1) = rho_i1;
  }
...
void Mesh::UserWorkAfterLoop(ParameterInput *pin)
{
 ...
 // Phii1Vec.DeleteAthenaArray();
 // Phii2Vec.DeleteAthenaArray();
 // Phii3Vec.DeleteAthenaArray();
 // AiiVec.DeleteAthenaArray();
...
void MeshBlock::ProblemGenerator(ParameterInput *pin)
{
 ...
 // Real iso_cs = pin->GetOrAddReal("problem","iso_sound_speed", .011);      // gas density outside cloud
  Real iso_cs = pin->GetOrAddReal("hydro","iso_sound_speed", .011);      // gas density outside cloud
//  std::cout << "iso_cs: " << iso_cs;
  // for tapered density profile 
  Real scaleheight = pin->GetOrAddReal("problem","scaleheight", .25);      // gas density outside cloud

 // std::cout << "cs in problem file: " << iso_cs;
  ...
  for(int k=ks; k<=ke; ++k) {
    for (int j=js; j<=je; ++j) {
      for (int i=is; i<=ie; ++i) {
        ...
        Real zcoord = pcoord->x3v(k);

        int xit=0;
        while((xit < int(rhoarr_len)) && (v(xit) < zcoord) ){
            ++xit;
          }
        //int xit = std::lower_bound(vect.begin(),vect.end(),x1);
        Real density = rhoarr(xit);
        Real Pcr = pc0*pow(density/rho_h,2./3.);
        Real pthermal = density*pow(iso_cs,2.);
        ...
        if (NON_BAROTROPIC_EOS){
          phydro->u(IEN,k,j,i) = pthermal/(gamma-1.0) + grav_acc*density*(pcoord->x3v(k))/(gamma-1.0);
         // phydro->u(IEN,k,j,i) = pgas/(gamma-1.0);
        }

        if(CR_ENABLED){
           pcr->u_cr(CRE,k,j,i) = (density*iso_cs*iso_cs/crBeta)/(1./3.);
           pcr->u_cr(CRF1,k,j,i) = 0.0;                       // no initial CR flux
           pcr->u_cr(CRF2,k,j,i) = 0.0;
           pcr->u_cr(CRF3,k,j,i) = 0.0;
           pcr->dedt_coul(k,j,i) = 0.0;                            // Chad
           pcr->dedt_pion(k,j,i) = 0.0;
        }
      }// end i
    }
  }
  ...
  if(CR_ENABLED){
    ...
    for(int k=0; k<nz3; ++k){
      for(int j=0; j<nz2; ++j){
        for(int i=0; i<nz1; ++i){
         // pcr->sigma_diff(0,k,j,i) = pcr->sigmaInput;
          pcr->sigma_diff(0,k,j,i) = 1./(pcr->diffInput);
         // pcr->sigma_diff(1,k,j,i) = sigma;
         // pcr->sigma_diff(2,k,j,i) = sigma;
          pcr->sigma_diff(1,k,j,i) = 1./(pcr->diffInput);  // isotropic diffusion
          pcr->sigma_diff(2,k,j,i) = 1./(pcr->diffInput);
  ...
  if(MAGNETIC_FIELDS_ENABLED){
    ...
    Real bx = pin->GetOrAddReal("problem","bx", 0.);
    Real by = pin->GetOrAddReal("problem","by", 0.);
    Real bz = pin->GetOrAddReal("problem","bz", 1.);
...
//          tot_cooling += pmb->user_out_var(0,k,j,i) * cell_vol(i);
	 ...
	 // Real gHeat = 1.e-25;
	  Real gHeat = 7.1e-25;   // gives a stable temp (for  n = 1) at T = 8000 K
...
#pragma omp simd
      for(int i=il; i<=iu; ++i){
       ...
       // pcr->sigma_diff(0,k,j,i) = pcr->sigmaInput;
        pcr->sigma_diff(0,k,j,i) = 1./(pcr->diffInput);
        pcr->sigma_diff(1,k,j,i) = 1./(pcr->diffInput);
        pcr->sigma_diff(2,k,j,i) = 1./(pcr->diffInput);
        //pcr->sigma_diff(1,k,j,i) = pcr->max_opacity;
        //pcr->sigma_diff(2,k,j,i) = pcr->max_opacity;  
...
#pragma omp simd
          for (int i=il; i<=iu; ++i) {
           // prim(IVZ,ku+k,j,i) = -prim(IVZ,ku-k+1,j,i);  // reflect 3-vel
            prim(IVZ,ku+k,j,i) = prim(IVZ,ku,j,i);  // do NOT reflect 3-vel
          }
        } else if (n==(IDN)) {
#pragma omp simd
          for (int i=il; i<=iu; ++i) {
...
#pragma omp simd
        for (int i=il; i<=iu; ++i) {
         // b.x3f((ku+k+1),j,i) = -b.x3f((ku-k+1),j,i);  // reflect 3-field
          b.x3f((ku+k),j,i) = b.x3f((ku-k+1),j,i);  // CRs probably won't play well with reflecting BC -- Chad
        }
      }
    }
  }
...
#pragma omp simd
        for (int k=1; k<=(NGHOST); ++k) {
          if (prim(IVZ,ke,j,i) < 0) {
            prim(IVZ,ke+k,j,i) = 0.0;  // diode
          } else {
            prim(IVZ,ke+k,j,i) = prim(IVZ,ke,j,i);  // copy
          }
        }
      } else {  
#pragma omp simd
      for (int k=1; k<=(NGHOST); ++k) {
        prim(n,ke+k,j,i) = prim(n,ke,j,i); // outflow
      }
      }
    }}
  }
...
#pragma simd
      for (int k=1; k<=(NGHOST); ++k) {
        b.x1f(ke+k+1,j,i) = b.x1f(ke+1,j,i);
      }
    }}
...
#pragma simd
      for (int k=1; k<=(NGHOST); ++k) {
        b.x2f(ke+k+1,j,i) = b.x2f(ke+1,j,i);
      }
    }}
     ...
     int ks, int ke, int ngh)
{
  ...
  Real pthermal = rho_h*pow(iso_cs,2.);
  for (int i=is; i<=ie; ++i) {
    for (int j=js; j<=je; ++j) {
      for (int k=1; k<=ngh; ++k) {

        prim(IDN,ks-k,j,i) = rho_h;
       // prim(IVX,k,j,is-i) = -prim(IVX,k,j,is); // reflect 1-velocity
        prim(IVX,ks-k,j,i) = prim(IVX,ks,j,i); // reflect 1-velocity
        prim(IVY,ks-k,j,i) = prim(IVY,ks,j,i);
        prim(IVZ,ks-k,j,i) = -prim(IVZ,ks,j,i);
       // prim(IVZ,ks-k,j,i) = prim(IVZ,ks,j,i);
        if(NON_BAROTROPIC_EOS)
          prim(IEN,ks-k,j,i) = pthermal + grav_acc*rho_h*(pco->x3v(k));
...
#pragma simd
        for (int k=1; k<=(NGHOST); ++k) {
          ...
          if(n==CRE) {
           ...
           // u_cr(n,ke+k,j,i) = 0.9999*u_cr(n,ke,j,i);
            u_cr(n,ke+k,j,i) = 0.9999*u_cr(n,ke,j,i);
          } else {
...
#pragma simd
        for (int k=1; k<=(NGHOST); ++k) {
          // enforce slight outward gradient to allow CRs to leave
          if(n==CRE) {
            Real slp = (u_cr(n,ke,j,i) - u_cr(n,ks,j,i))/((ke-ks)*pco->dx3f(k));
            u_cr(n,ke+k,j,i) = u_cr(n,ke,j,ie) + slp*k*pco->dx3f(k);
      //      int xit=0;
      //      Real zcoord = zmax + (k)*(pco->dx3f(k));
      //      while((xit < int(rhoarr_len)) && (v(xit) < zcoord)){
      //        ++xit;
      //      }
      //      u_cr(n,ke+k,j,i) = 3.0*rhoarr(xit)*iso_cs*iso_cs/crBeta;
            //u_cr(n,ke+k,j,i) = 0.9*u_cr(n,ke,j,i);
           // u_cr(n,ke+k,j,i) = u_cr(n,ke+k-1,j,i) - u_cr(CRF3,ke+k-1,j,i)*dt/(pco->dx3f(k));
           // cout << "dz: " << pco->dx3f(k) << " u_cr inside: " << u_cr(n,ke+k-1,j,i) << " u_cr outside: " << u_cr(n,ke+k,j,i) << endl;
          } else {
            u_cr(n,ke+k,j,i) = u_cr(n,ke,j,i);
          }
        }
...
#pragma simd
          for (int k=1; k<=(NGHOST); ++k) {
            u_cr(n,ks-k,j,i) = 3.0*(rho_h*iso_cs*iso_cs/crBeta);  // copy everything else as for hydro variables
           // u_cr(CRF1,ks-k,j,i) = u_cr(CRF1,ks,j,i);  // set x-crflux to set value
          }
        }}
      }
      else {
...
#pragma simd
          for (int k=1; k<=(NGHOST); ++k) {
            ...
            u_cr(n,ks-k,j,i) = u_cr(n,ks,j,i);  // set x-crflux to set value
           // u_cr(n,ks-k,j,i) = (rho_h*iso_cs*iso_cs/crBeta);  // copy everything else as for hydro variables
          }
        }}
      }
    }
