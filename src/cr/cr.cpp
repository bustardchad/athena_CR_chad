...
// constructor overload resolution of non-aggregate class type AthenaArray<Real>
    flux{ {NCR, pmb->ncells3, pmb->ncells2, pmb->ncells1+1},
      ...
      {NCR,pmb->ncells3+1, pmb->ncells2, pmb->ncells1,
       (pmb->pmy_mesh->f3 ? AthenaArray<Real>::DataStatus::allocated :
        AthenaArray<Real>::DataStatus::empty)}}, UserSourceTerm_{}, UserSourceTerm2_{},
  ...
  flux_t = pin->GetOrAddReal("problem","flux_t",0.);   // added by JW
  fluxdt = pin->GetOrAddReal("problem","fluxdt",1.);   // added by JW
  
 // sigma = pin->GetOrAddReal("cr","sigma",1.e30);   // added by JW 
  maxNeut = pin->GetOrAddReal("cr","maxNeut",0.999999);   // added by Chad
  ...
  NLLD = pin->GetOrAddInteger("cr","NLLD",0);   // added by Chad
  use_piso = pin->GetOrAddInteger("cr","use_piso",0);   // added by Chad
 ...
 // q_cr.NewAthenaArray(n3z,n2z,n1z);   // added by JW - stores CR source term in each cell
 // dedt_shock.NewAthenaArray(nc3,nc2,nc1);   // added by Chad - stores CR source term in each cell
  dedt_coul.NewAthenaArray(nc3,nc2,nc1);   // added by Chad - stores CR source term in each cell
  dedt_pion.NewAthenaArray(nc3,nc2,nc1);   // added by Chad - stores CR source term in each cell
  dedt_damping.NewAthenaArray(nc3,nc2,nc1);   // added by Chad - stores CR source term in each cell
  crscale.NewAthenaArray(nc3,nc2,nc1);   // added by Chad - stores CR source term in each cell
  bscale.NewAthenaArray(nc3,nc2,nc1);   // added by Chad - stores CR source term in each cell
  uncapped_diffusion.NewAthenaArray(nc3,nc2,nc1);   // added by Chad - stores CR source term in each cell
  vgradpc.NewAthenaArray(nc3,nc2,nc1);   // added by Chad -- flow velocity dot grad Pc, not vA dot grad Pc

 // UserSourceTerm2_.NewAthenaArray(nc3,nc2,nc1);
...
void CosmicRay::EnrollUserCRGasSource(CRGasSrcTermFunc my_CRGasFunction)
{
  UserSourceTerm2_ = my_CRGasFunction;
 // UserSourceTerm_ = my_CRGasFunction;
  cr_source_defined = true;
