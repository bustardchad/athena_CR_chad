...
def collisional(field,data): #was defined as negative in Athena++, so needs a minus sign
          return (1.022e-15)*(data['d']/1.67e-24) * (data['Ec']*edenstocgs) * YTQuantity(1,"erg/s/g")
...
yt.add_field(("gas","collisional"), function=collisional,display_name="Collisional Loss",units="erg/cm**3/s")
...
ts = yt.DatasetSeries('../cr.out1.00030*',parallel=10)
for ds in ts.piter():
 dd = ds.all_data()
