...
profiles.append(yt.create_profile(dd, "d", ["ecr"],weight_field=None, fractional=True, accumulation=False,extrema={'ecr': (1e-3, 1e0),
                                      'd': (1e-27, 1e-21)}))

# Add labels
labels.append(r"$v_{st} = v_{A}$")
...
# Add labels
labels.append(r"$v_{st} = v_{A}^{ion}$ + Ion-Neutral Damping")
