...
#if (CR_ENABLED > 0)
  #define NCRV (4)
 // #define NCRV (5)  // Chad, also see below with fixes to CR output
#else
  #define NCRV 0
#endif
...
void HistoryOutput::WriteOutputFile(Mesh *pm, ParameterInput *pin, bool flag) {
  ...
  const int nhistory_output = NHISTORY_VARS + pm->nuser_history_output_ + NRAD;
      ...
      for (int j=pmb->js; j<=pmb->je; ++j) {
        ...
        for (int i=pmb->is; i<=pmb->ie; ++i) {
          ...
          if(CR_ENABLED){
           ...
           // hst_data[prev_out + 4] += vol(i)*sqrt(SQR(pcr->dedt_damping(k,j,i)));
